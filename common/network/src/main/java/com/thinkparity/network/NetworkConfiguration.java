...
import com.thinkparity.network.log4j.or.NetworkAddressRenderer;
import com.thinkparity.network.log4j.or.NetworkConnectionRenderer;
import com.thinkparity.network.log4j.or.NetworkProtocolRenderer;
import com.thinkparity.network.log4j.or.NetworkProxyRenderer;
import com.thinkparity.network.log4j.or.NetworkRenderer;
...
public final class NetworkConfiguration {
    ...
    /** The per-proxy configuration. */
    private static Map<NetworkProxy, Map<String, Object>> perProxy;
     ...
     */
    public Integer getConnectTimeout(final NetworkProtocol protocol,
            final NetworkAddress address, final NetworkProxy proxy) {
        return (Integer) get(protocol, address, proxy,
                Names.Network.Connection.CONNECT_TIMEOUT);
    }
    ...
    public Integer getConnectTimeout(final NetworkProtocol protocol,
        ...
        return (Integer) get(protocol, address, proxy, connection,
                Names.Network.Connection.CONNECT_TIMEOUT);
    }
    ...
    public Boolean getTcpNoDelay(final NetworkProtocol protocol,
            final NetworkAddress address, final NetworkProxy proxy) {
        return (Boolean) get(protocol, address, proxy,
                Names.Network.Connection.TCP_NODELAY);
    }
    ...
    public Boolean getTcpNoDelay(final NetworkProtocol protocol,
            final NetworkAddress address, final NetworkProxy proxy,
            final NetworkConnection connection) {
        return (Boolean) get(protocol, address, proxy, connection,
                Names.Network.Connection.TCP_NODELAY);
    }
     ...
     *            An <code>Integer</code>.
     */
    public void setConnectTimeout(final NetworkProxy proxy,
            final Integer connectTimeout) {
        set(proxy, Names.Network.Connection.CONNECT_TIMEOUT, connectTimeout);
    }
    ...
    private synchronized Object get(final NetworkProtocol protocol,
            final NetworkAddress address, final NetworkProxy proxy,
            final NetworkConnection connection, final String name) {
        if (null == perConnection) {
            return get(protocol, address, proxy, name);
        } else {
    ...
    /**
     * Obtain a proxy; address; protocol configuration.
     * 
     * @param protocol
     *            A <code>NetworkProtocol</code>.
     * @param address
    ...
    private synchronized Object get(final NetworkProtocol protocol,
        ...
        if (null == perProxy) {
            ...
            if (perProxy.containsKey(proxy)) {
                final Map<String, Object> values = perProxy.get(proxy);
                if (values.containsKey(name)) {
                    LOGGER.logInfo("Get \"{0}\":\"{1}\" for {2}.", name,
                            values.get(name), proxy);
                    return values.get(name);
                } else {
                    return get(protocol, address, name);
                }
     ...
     *            An <code>Object</code>.
     */
    private synchronized void set(final NetworkProxy proxy,
            final String name, final Object value) {
        if (null == perProxy) {
            perProxy= new Hashtable<NetworkProxy, Map<String, Object>>();
            perProxy.put(proxy, new Hashtable<String, Object>());
        }
        ensureEntry(perProxy, proxy);
        LOGGER.logInfo("Set \"{0}\":\"{1}\" for {2}.", name, value, proxy);
        perProxy.get(proxy).put(name, value);
    }
    ...
    private static final class Names {
        ...
        private static final class Network {
            ...
            private static final class Connection {
                ...
                private static final String PROXIES = "network.connection.proxies";
