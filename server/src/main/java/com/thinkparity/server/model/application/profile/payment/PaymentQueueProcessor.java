 ...
 * @version 1.1.2.1
 */
class PaymentQueueProcessor implements Runnable {
    ...
    public void run() {
        ...
        while (run) {
            ...
            if (run) {
                logger.logTraceId();
                logger.logInfo("Processing payment queue.");
                try {
                    modelFactory.getProfileModel().processPaymentQueue();
                    errCount = 0;
                } catch (final Throwable t) {
                    logger.logError(t, "Could not process payment queue.");
                    errCount++;
    ...
    /**
     * Determine if the processor is stopped.
     * 
     * @return True if the processor is running.
     */
    Boolean isStopped() {
