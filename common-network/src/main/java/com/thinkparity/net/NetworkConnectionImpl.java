...
class NetworkConnectionImpl implements NetworkConnection {
    ...
    /**
     * @see com.thinkparity.net.NetworkConnection#connect()
     *
     */
    @Override
    public void connect() throws NetworkException {
        ...
        if (null == proxy) {
            proxy = networkImpl.getConfiguration().getProxy(this);
        }
    ...
    /**
     * @see com.thinkparity.net.NetworkConnection#connect(com.thinkparity.net.NetworkProxy)
     *
     */
    @Override
    public void connect(final NetworkProxy proxy) throws NetworkException {
        logger.logTraceId();
        this.proxy = proxy;
        connect();
    }
    ...
    /**
     * @see com.thinkparity.net.NetworkConnection#connect(com.thinkparity.net.NetworkTunnel)
     *
     */
    @Override
    public void connect(final NetworkTunnel tunnel) throws NetworkException {
        logger.logTraceId();
        this.tunnel = tunnel;
        connect();        
    }
    ...
    /**
     * @see com.thinkparity.net.NetworkConnection#disconnect()
     *
     */
    @Override
    ...
    /**
     * @see com.thinkparity.net.NetworkConnection#getAddress()
     *
     */
    ...
    /**
     * @see com.thinkparity.net.NetworkConnection#getId()
     *
     */
    @Override
    public String getId() {
    ...
    /**
     * @see com.thinkparity.net.NetworkConnection#getSocket()
     *
     */
    @Override
    ...
    /**
     * @see com.thinkparity.net.NetworkConnection#isConnected()
     *
     */
    @Override
    ...
    /**
     * @see com.thinkparity.net.NetworkConnection#read(byte[])
     *
     */
    @Override
    ...
    /**
     * @see com.thinkparity.net.NetworkConnection#write(byte[])
     *
     */
    @Override
     ...
     * @throws IOException
     */
    private void connectSocket() throws NetworkException, SocketException,
            IOException {
        socket = newSocket();
        if (Boolean.FALSE == protocol.isSecure()) {
    ...
    /**
     * Ensure the tunnel is connected.
     * 
     */
    private void ensureTunnelConnected() {
        ensureTunnelSet();
        if (null == tunnel.getSocket()) {
            throw new IllegalStateException(getId() + " - Network tunnel socket is not set.");
        }
        if (false == tunnel.getSocket().isConnected()) {
            throw new IllegalStateException(getId() + " - Network tunnel socket is not connected.");
        }
    }

    /**
     * Ensure the tunnel is set.
     * 
     */
    private void ensureTunnelSet() {
        if (null == tunnel) {
            throw new IllegalStateException(getId() + " - Network tunnel is not set.");
        }
    }
    ...
    private Integer getConnectTimeout() {
        return getConfiguration().getConnectTimeout(protocol, address, this);
    }
    ...
    private Socket newSocket() throws NetworkException, UnknownHostException {
        if (protocol.isSecure()) {
            if (null == tunnel) {
                return networkImpl.newSecureSocket(id, address);
            } else {
                ensureTunnelConnected();
                return networkImpl.newSecureSocket(id, address, tunnel);
            }
        } else {
            if (null == proxy) {
                return networkImpl.newSocket(id);
            } else {
    ...
    private void setSocketOptions() throws SocketException {
        ...
        final Integer soLinger = configuration.getSoLinger(protocol, address, this);
        ...
        final Integer soTimeout = configuration.getSoTimeout(protocol, address, this);
        logger.logDebug("{0} - soTimeout:{1}", getId(), soTimeout);
        final Boolean tcpNoDelay = configuration.getTcpNoDelay(protocol, address, this);
