...
class PackageTestWarTask {
    ...
    void execute() {
        ...
        def testClassesDir = configuration["thinkparity.target.test-classes-dir"]
        ...
        ant.sequential {
            ...
            copy(todir:warLibDir) {
                ...
                fileset(refid:"test.dependencies-java")
                mapper(type:"flatten")
            }
            ...
            jar(jarfile:new File(warLibDir,"tps-model-test.jar"),duplicate:"fail",update:"true",whenempty:"fail") {
                fileset(dir:testClassesDir)
            }
            // war
...
// execute package
new PackageTestWarTask(ant:new AntBuilder(project),properties:properties).execute()
