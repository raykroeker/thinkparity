 ...
 * @version 1.1
 */
public class IQHandleReactivateProvider extends IQParityProvider {

    /** Create IQHandleReactivateProvider. */
    public IQHandleReactivateProvider() { super(); }
    ...
    public IQ parseIQ(final XmlPullParser parser) throws Exception {
        ...
        logger.debug(parser);
        final IQHandleReactivate iq = new IQHandleReactivate();
        ...
        while(Boolean.FALSE == isComplete) {
            ...
            else if(XmlPullParser.START_TAG == eventType && "jabberIds".equals(name)) {
                parser.next();
            }
            else if(XmlPullParser.END_TAG == eventType && "jabberIds".equals(name)) {
                parser.next();
            }
            else if(XmlPullParser.END_TAG == eventType && "jabberId".equals(name)) {
                parser.next();
                iq.addTeamMember(JabberIdBuilder.parseQualifiedJabberId(parser.getText()));
                parser.next();
            }
            else if(XmlPullParser.END_TAG == eventType && "jabberId".equals(name)) {
                parser.next();
