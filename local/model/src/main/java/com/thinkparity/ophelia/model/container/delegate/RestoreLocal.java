...
/**
 * <b>Title:</b>thinkParity OpheliaModel Container Restore Local Delegate<br>
 * <b>Description:</b><br>
 * 
 * @author raymond@thinkparity.com
 * @version 1.1.2.1
 */
public final class RestoreLocal extends ContainerDelegate {

    /** A <code>ProcessMonitor</code>. */
    private ProcessMonitor monitor;

    /** The monitor data. */
    private final RestoreLocalData monitorData;
    ...
    public RestoreLocal() {
        super();
        this.monitorData = new RestoreLocalData();
    }

    /**
     * Restore local data from the backup.
     * 
     ...
     *             if a decryption error occurs
     */
    public void restoreLocal() throws CannotLockException, NetworkException,
        ...
        notifyStepBegin(monitor, RestoreLocalStep.RESTORE_CONTAINERS, monitorData);
        for (final Container backupContainer : backupContainers) {
            monitorData.setRestoreContainer(backupContainer);
            notifyStepBegin(monitor, RestoreLocalStep.RESTORE_CONTAINER, monitorData);
            ...
            notifyStepEnd(monitor, RestoreLocalStep.RESTORE_CONTAINER);
        }
        notifyStepEnd(monitor, RestoreLocalStep.RESTORE_CONTAINERS);
        /* restore */
        notifyStepBegin(monitor, RestoreLocalStep.FINALIZE_RESTORE, monitorData);
        containerService.confirmRestoreBackup(getAuthToken(), restoredOn);
        notifyStepEnd(monitor, RestoreLocalStep.FINALIZE_RESTORE);
    }
    ...
    private CryptoMonitor newCryptoMonitor() {
        return new CryptoMonitor() {
            ...
            public void chunkDecrypted(final int chunkSize) {
                ...
                notifyStepBegin(monitor, RestoreLocalStep.RESTORE_DOCUMENT_VERSION_DECRYPT_BYTES, monitorData);
                notifyStepEnd(monitor, RestoreLocalStep.RESTORE_DOCUMENT_VERSION_DECRYPT_BYTES);
            }
            /**
    ...
    private StreamMonitor newStreamMonitor(final DocumentVersion version) {
        return new StreamMonitor() {
            /**
             * @see com.thinkparity.stream.StreamMonitor#chunkReceived(int)
             *
             */
            @Override
            public void chunkReceived(final int chunkSize) {
                monitorData.setBytes(chunkSize);
                notifyStepBegin(monitor, RestoreLocalStep.RESTORE_DOCUMENT_VERSION_DOWNLOAD_BYTES, monitorData);
                notifyStepEnd(monitor, RestoreLocalStep.RESTORE_DOCUMENT_VERSION_DOWNLOAD_BYTES);
            }
            /**
             * @see com.thinkparity.stream.StreamMonitor#chunkSent(int)
             *
             */
            @Override
            ...
            /**
             * @see com.thinkparity.stream.StreamMonitor#getName()
             *
             */
            @Override
            ...
            /**
             * @see com.thinkparity.stream.StreamMonitor#reset()
             *
             */
            @Override
            public void reset() {
                monitorData.setBytes(0);
                notifyStepBegin(monitor, RestoreLocalStep.RESET_RESTORE_DOCUMENT_VERSION, monitorData);
                notifyStepEnd(monitor, RestoreLocalStep.RESET_RESTORE_DOCUMENT_VERSION);
            }
        };
    }
    ...
    private void restore(final Container container) throws NetworkException,
        ...
        for (final ContainerVersion version : versions) {
            ...
            notifyStepBegin(monitor, RestoreLocalStep.RESTORE_DOCUMENTS, monitorData);
            ...
            for (final Document document : documents) {
                ...
                notifyStepBegin(monitor, RestoreLocalStep.RESTORE_DOCUMENT_VERSIONS, monitorData);
                for (final DocumentVersion documentVersion : documentVersions) {
                    if (documentVersion.getArtifactUniqueId().equals(document.getUniqueId())) {
                        if (!artifactIO.doesVersionExist(document.getId(),
                            ...
                            notifyStepBegin(monitor, RestoreLocalStep.RESTORE_DOCUMENT_VERSION, monitorData);
                            ...
                            notifyStepEnd(monitor, RestoreLocalStep.RESTORE_DOCUMENT_VERSION);
                            break;
                        } else {
                            containerIO.addVersion(container.getId(),
                                    version.getVersionId(), document.getId(),
                ...
                notifyStepEnd(monitor, RestoreLocalStep.RESTORE_DOCUMENT_VERSIONS);
            ...
            notifyStepEnd(monitor, RestoreLocalStep.RESTORE_DOCUMENTS);
